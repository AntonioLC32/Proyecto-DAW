...
export default defineComponent({
  ...
  mounted() {
    this.randomIdRoulette = Number(
      (Math.random() * (999999 - 1) + 1).toFixed(0)
    );
    this.$nextTick(() => {
      this.reset();
      document
...
.wheel-base-container {
  ...
  .wheel-base-indicator:before {
    ...
    border-left: 20px solid transparent;
    border-right: 20px solid transparent;
    border-bottom: 20px solid cac5f5;
    transform: translateX(-50%);
  }
}
...
.wheel-center-indicator {
  ...
  border-bottom: 20px solid #4a4da5;
  transform: translate(-50%, -50%);
  pointer-events: none;
}
...
.wheel {
  ...
  &-border:after {
    ...
    -webkit-mask: radial-gradient(transparent 65%, #cac5f5 66%);
    mask: radial-gradient(transparent 65%, #cac5f5 66%);
  }
